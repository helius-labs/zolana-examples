...
The maker and taker co-sign one private SOL/SPL swap without escrow or a custom program.

Copied from [`sdk-tests/rfq`](https://github.com/helius-labs/zolana/tree/ebca3ad1bd2f27b1f1f04e33fbfa3cc4c7bbf856/sdk-tests/rfq) at `ebca3ad1bd2f27b1f1f04e33fbfa3cc4c7bbf856`.
...
```bash
scripts/program-examples.sh test rfq
```
...
```bash
scripts/program-examples.sh bench rfq
```

At the pinned source revision, this ignored benchmark fails with `InvalidTreeAccounts` (`Custom(7001)`). The same failure reproduces in the unchanged Zolana source. The regular localnet settlement test passes; the benchmark fixture is preserved here.
