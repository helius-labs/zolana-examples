...
The stateless reader creates an SDK wallet only for the duration of a request. Decrypted notes, history, and scan cursors are not persisted or reused across operations. React retains displayed balances and receipts. This does not change Turnkey login persistence or the TVC recovery records described above. Per-transaction viewing keys returned by TVC are temporary browser objects and are destroyed after encryption; decryption results and transaction proof inputs are still processed in the app.
